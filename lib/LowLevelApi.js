...
var NotFoundError = require('./exceptions/NotFoundError.js').NotFoundError;
var AccessDeniedError = require('./exceptions/AccessDeniedError.js').AccessDeniedError;
...
LowLevelApi.prototype.request = function(relativeUrl, parameters, onComplete) {
	...
	function createError(message, result) {
	...
	function onBodyCompleted() {
		try {
			...
			if(result.error) {
				result.error.code = parseInt(status);
				throw createError(result.error.message, result);
			}
